...
public class TimeAsInt {
    ...
    public static TimeAsInt addSecs(TimeAsInt obj, int secs){
      return new TimeAsInt(0,0, obj.value + secs);
    }

    public static TimeAsInt addMinutes(TimeAsInt obj, int min,int secs){
      int timeToAdd = min * 60 + secs;
      return new TimeAsInt(0,0, obj.value + timeToAdd);
    }

    public static TimeAsInt addHours(TimeAsInt obj, int hour,int min,int secs){
      return new TimeAsInt(0,0, obj.value + timeToSeconds(hour, min, secs));
    }
    ...
    public boolean equals(Object obj){
      return compareTimeEquality((TimeAsInt) obj);
    }
    
    private boolean compareTimeEquality(TimeAsInt obj){
      return this.getTimeAsSeconds() == obj.getTimeAsSeconds();
    }
    ...
    private static int timeToSeconds(int hora, int min, int sec){
